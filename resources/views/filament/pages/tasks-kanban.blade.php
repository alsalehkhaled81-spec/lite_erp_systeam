            ...
            <div class="flex-shrink-0 w-80 rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-lg"
                ...
                <div class="space-y-3 p-4 min-h-[120px]">
                    @foreach($status['records'] as $task)
                        <a href="{{ \App\Filament\Pm\Resources\TaskResource::getUrl('edit', ['record' => $task->id]) }}" class="block group rounded-xl p-4 transition-all duration-200 cursor-pointer hover:-translate-y-0.5 hover:shadow-md
                                    bg-white/80 dark:bg-gray-800/60 border border-gray-100/80 dark:border-gray-700/40
                                    backdrop-blur-sm">
                            <div class="font-semibold text-sm text-gray-800 dark:text-gray-100 mb-1.5 group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors">
                                {{ $task->title }}
                            </div>
